...
export default function* checkoutAuction(api) {
  while (true) {
    yield take(types.CHECKOUT_AUCTION_BUY_START)
    ...
    yield put(actions.checkoutAuction.checkoutAuctionBuySuccess())
    yield put(actions.leads.setSelectedLeads("AUCTION_LEADS", new Set()))
    window.triggerFetch()
...
const leadTransfer = function*(api, lead, user) {
  try {
    ...
    yield put(actions.checkoutAuction.checkoutAuctionBuyError(err.message))
  }
}
