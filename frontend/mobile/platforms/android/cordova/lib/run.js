...
module.exports.run = function(runOptions) {
  ...
  return Q()
      ...
      if (!install_target) {
        ...
        return device.list().then(function(device_list) {
          if (device_list.length > 0) {
            events.emit(
              "warn",
              "No target specified, deploying to device '" +
                device_list[0] +
                "'.",
            )
        ...
        return emulator.list_started().then(function(started_emulators) {
          ...
          return emulator.list_images().then(function(avds) {
            ...
            return Q.reject(
              "Target '" +
                install_target +
                "' not found, unable to run project",
            )
          })
        })
      })
    ...
    .then(function(resolvedTarget) {
      ...
      return build.run
        .call(self, runOptions, resolvedTarget)
        .then(function(buildResults) {
          if (resolvedTarget.isEmulator) {
            return emulator
              .wait_for_boot(resolvedTarget.target)
              .then(function() {
...
module.exports.help = function() {
  console.log(
    "Usage: " + path.relative(process.cwd(), process.argv[1]) + " [options]",
  )
  console.log("Build options :")
  console.log("    --debug : Builds project in debug mode")
  console.log("    --release : Builds project in release mode")
  console.log(
    "    --nobuild : Runs the currently built project without recompiling",
  )
  console.log("Deploy options :")
  console.log("    --device : Will deploy the built project to a device")
  console.log(
    "    --emulator : Will deploy the built project to an emulator if one exists",
  )
  console.log(
    "    --target=<target_id> : Installs to the target with the specified id.",
  )
  process.exit(0)
}
