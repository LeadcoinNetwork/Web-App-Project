...
module.exports.prepare = function(cordovaProject, options) {
  ...
  this._config = updateConfigFilesFrom(
  ...
  return Q.when(updateWww(cordovaProject, this.locations))
    ...
    .then(function() {
      updateIcons(
        cordovaProject,
        path.relative(cordovaProject.root, self.locations.res),
      )
      updateSplashes(
        cordovaProject,
        path.relative(cordovaProject.root, self.locations.res),
...
module.exports.clean = function(options) {
  ...
  return Q().then(function() {
    ...
    cleanIcons(
      projectRoot,
      projectConfig,
      path.relative(projectRoot, self.locations.res),
    )
    cleanSplashes(
      projectRoot,
      projectConfig,
...
function updateProjectAccordingTo(platformConfig, locations) {
  ...
  if (shortName && shortName !== name) {
    strings.find('string[@name="launcher_name"]').text = shortName.replace(
      /\'/g,
      "\\'",
    )
  }
  ...
  manifest
    ...
    .setVersionCode(
      platformConfig.android_versionCode() ||
        default_versionCode(platformConfig.version()),
    )
    .setPackageId(androidPkgName)
    .setMinSdkVersion(
      platformConfig.getPreference("android-minSdkVersion", "android"),
    )
    .setMaxSdkVersion(
      platformConfig.getPreference("android-maxSdkVersion", "android"),
    )
    .setTargetSdkVersion(
      platformConfig.getPreference("android-targetSdkVersion", "android"),
    )
    .write()

  // Java file paths shouldn't be hard coded
  var javaPattern = path.join(
    locations.javaSrc,
  ...
  shell
    .sed(/package [\w\.]*;/, "package " + androidPkgName + ";", java_files[0])
    .to(destFile)
  events.emit(
    "verbose",
...
function mapImageResources(rootDir, subDir, type, resourceName) {
  var pathMap = {}
  shell
    .ls(path.join(rootDir, subDir, type + "-*"))
    .forEach(function(drawableFolder) {
      var imagePath = path.join(
        subDir,
        path.basename(drawableFolder),
        resourceName,
      )
