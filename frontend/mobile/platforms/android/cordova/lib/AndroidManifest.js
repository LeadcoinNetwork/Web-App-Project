...
function AndroidManifest(path) {
  ...
  if (this.doc.getroot().tag !== "manifest") {
    throw new Error(
      "AndroidManifest at " +
        path +
        ' has incorrect root node name (expected "manifest")',
    )
  }
}
...
;["minSdkVersion", "maxSdkVersion", "targetSdkVersion"].forEach(function(
  sdkPrefName,
) {
  ...
  AndroidManifest.prototype["set" + capitalize(prefName)] = function(
    prefValue,
  ) {
    var usesSdk = this.doc.getroot().find("./uses-sdk")
...
AndroidManifest.prototype.getDebuggable = function() {
  return (
    this.doc.getroot().find("./application").attrib["android:debuggable"] ===
    "true"
  )
}
