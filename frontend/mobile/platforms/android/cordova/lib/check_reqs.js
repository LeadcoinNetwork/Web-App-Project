...
function tryCommand(cmd, errMsg, catchStderr) {
  ...
  child_process.exec(cmd, function(err, stdout, stderr) {
    if (err) d.reject(new CordovaError(errMsg))
      ...
      d.resolve((catchStderr ? stderr : stdout).trim())
  })
  return d.promise
}
...
module.exports.get_target = function() {
  ...
  throw new Error(
    "Could not find android target in either " +
      repo_file +
      " nor " +
      project_file,
  )
    ...
    .catch(function(err) {
      if (err) {
        throw new CordovaError(
          "Failed to run `ant -version`. Make sure you have `ant` on your $PATH.",
        )
      }
    })
}
  ...
  return Q()
    .then(function() {
      if (hasJavaHome) {
        ...
        if (!javacPath) {
          process.env["PATH"] +=
            path.delimiter + path.join(process.env["JAVA_HOME"], "bin")
        }
      } else {
        if (javacPath) {
          // OS X has a command for finding JAVA_HOME.
          var find_java = "/usr/libexec/java_home"
          var default_java_error_msg =
          ...
          if (firstJdkDir) {
            ...
            if (!javacPath) {
              process.env["PATH"] +=
                path.delimiter + path.join(firstJdkDir, "bin")
            }
    ...
    .then(function() {
      ...
      return tryCommand("javac -version", msg, true).then(function(output) {
        ...
        var match = /javac ((?:1\.)(?:[8-9]\.)(?:\d+))|((?:1\.)(?:[1-9]\d+\.)(?:\d+))/i.exec(
          output,
        )
  ...
  return Q().then(function() {
    ...
    if (!hasAndroidHome && !androidCmdPath && !adbInPath && !avdmanagerInPath) {
      ...
      if (process.env["HOME"]) {
        ...
        maybeSetAndroidHome(
          path.join(process.env["HOME"], "android-sdk-macosx"),
        )
    ...
    if (!hasAndroidHome) {
      ...
      if (androidCmdPath) {
        ...
        if (
          path.basename(parentDir) === "tools" ||
          fs.existsSync(path.join(grandParentDir, "tools", "android"))
        ) {
    ...
    if (hasAndroidHome && !androidCmdPath) {
      process.env["PATH"] +=
        path.delimiter + path.join(process.env["ANDROID_HOME"], "tools")
    }
    ...
    if (hasAndroidHome && !avdmanagerInPath) {
      process.env["PATH"] +=
        path.delimiter + path.join(process.env["ANDROID_HOME"], "tools", "bin")
    }
