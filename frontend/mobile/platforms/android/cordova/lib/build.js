...
function parseOpts(options, resolvedTarget, projectRoot) {
  ...
  if (options.argv.keystore) {
    packageArgs.keystore = path.relative(
      projectRoot,
      path.resolve(options.argv.keystore),
    )
  }

  ;["alias", "storePassword", "password", "keystoreType"].forEach(function(
    flagName,
  ) {
  ...
  if (buildConfig) {
    ...
    events.emit(
      "log",
      "Reading build config file: " + path.resolve(buildConfig),
    )
      ...
      if (androidInfo.keystore && !packageArgs.keystore) {
        if (androidInfo.keystore.substr(0, 1) === "~") {
          androidInfo.keystore =
            process.env.HOME + androidInfo.keystore.substr(1)
        }
...
module.exports.findBestApkForArchitecture = function(buildResults, arch) {
  ...
  throw new Error(
    "Could not find apk architecture: " +
      arch +
      " build-type: " +
      buildResults.buildType,
  )
}
...
module.exports.help = function() {
  ...
  console.log(
    "    '--prepenv': don't build, but copy in build scripts where necessary",
  )
  console.log(
    "    '--versionCode=#': Override versionCode for this build. Useful for uploading multiple APKs. Requires --gradle.",
  )
  console.log(
    "    '--minSdkVersion=#': Override minSdkVersion for this build. Useful for uploading multiple APKs. Requires --gradle.",
  )
  console.log(
  ...
  console.log(
    "    '--storePassword=': Password for the key store. (Optional - prompted)",
  )
