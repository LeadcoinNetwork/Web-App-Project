          ...
          if (response[i + 1].match(/Based\son:\s/)) {
            ...
            if (img_obj["target"].match(/Tag\/ABI:\s/)) {
              ...
              if (img_obj["target"].indexOf("(") > -1) {
                img_obj["target"] = img_obj["target"]
                  .substr(0, img_obj["target"].indexOf("(") - 1)
                  .trim()
              }
            }
            ...
            var api_level =
              android_sdk.version_string_to_api_level[version_string]
            if (api_level) {
  ...
  return superspawn.spawn("android", ["list", "avd"]).then(function(output) {
    ...
    for (var i = 1; i < response.length; i++) {
      ...
      if (response[i].match(/Name:\s/)) {
        ...
        if (
          response[i + 1].match(/\(API\slevel\s/) ||
          (response[i + 2] && response[i + 2].match(/\(API\slevel\s/))
        ) {
          i++
          var secondLine = response[i + 1].match(/\(API\slevel\s/)
            ? response[i + 1]
            : ""
          img_obj["target"] = (response[i] + secondLine)
            .split("Target: ")[1]
            .replace("\r", "")
        }
        if (response[i + 1].match(/ABI:\s/)) {
...
module.exports.list_images = function() {
  return Q.fcall(function() {
    ...
    return avds.map(function(avd) {
      if (
        avd.target &&
        avd.target.indexOf("Android API") > -1 &&
        avd.target.indexOf("API level") < 0
      ) {
        var api_level = avd.target.match(/\d+/)
        if (api_level) {
          var level = android_versions.get(api_level)
          avd.target =
            "Android " + level.semver + " (API level " + api_level + ")"
        }
      }
      return avd
    })
...
module.exports.best_image = function() {
  return this.list_images().then(function(images) {
    ...
    var closest = 9999
    var best = images[0]
    var project_target = parseInt(
      check_reqs.get_target().replace("android-", ""),
    )
    for (var i in images) {
...
module.exports.list_targets = function() {
  return superspawn
    .spawn("android", ["list", "targets"], { cwd: os.tmpdir() })
...
module.exports.start = function(emulator_ID, boot_timeout) {
  ...
  return Q()
    .then(function() {
      ...
      return self.best_image().then(function(best) {
        if (best && best.name) {
          events.emit(
            "warn",
            "No emulator specified, defaulting to " + best.name,
          )
          return best.name
        }
    ...
    .then(function(emulatorId) {
      ...
      return self
        .wait_for_boot(emulatorId, boot_timeout)
...
module.exports.wait_for_boot = function(emulator_id, time_remaining) {
  ...
  return Adb.shell(emulator_id, "ps").then(function(output) {
    if (output.match(/android\.process\.acore/)) {
      ...
      return Q.delay(
        time_remaining < CHECK_BOOTED_INTERVAL
          ? time_remaining
          : CHECK_BOOTED_INTERVAL,
      ).then(function() {
        var updated_time =
          time_remaining >= 0
            ? Math.max(time_remaining - CHECK_BOOTED_INTERVAL, 0)
            : time_remaining
  ...
  if (target) {
    return superspawn
      ...
      .then(null, function(error) {
        ...
        console.error(
          " Do you have the latest android targets including " + target + "?",
        )
        console.error(error)
      })
  } else {
...
module.exports.resolveTarget = function(target) {
  return this.list_started().then(function(emulator_list) {
    if (emulator_list.length < 1) {
      return Q.reject(
        "No running Android emulators found, please start an emulator before deploying your project.",
      )
    }
    ...
    if (emulator_list.indexOf(target) < 0) {
      return Q.reject(
        "Unable to find target '" + target + "'. Failed to deploy to emulator.",
      )
    }
  ...
  return Q()
    ...
    .then(function() {
      ...
      return Q.when().then(function() {
        var apk_path = build.findBestApkForArchitecture(
          buildResults,
          target.arch,
        )
        var execOptions = {
          ...
          return Q.promise(function(resolve, reject) {
            child_process.exec(command, opts, function(err, stdout, stderr) {
              if (err)
                ...
                reject(
                  new CordovaError(
                    "Failed to install apk to emulator: " + stdout,
                  ),
                )
              } else resolve(stdout)
            })
        ...
        function installPromise() {
          return adbInstallWithOptions(
            ...
            if (
              !/INSTALL_PARSE_FAILED_INCONSISTENT_CERTIFICATES/.test(
                error.toString(),
              )
            ) {
              throw error
        ...
        return retry
          .retryPromise(NUM_INSTALL_RETRIES, installPromise)
          .then(function(output) {
    ...
    .then(function() {
      Adb.start(
        target.target,
        pkgName + "/." + manifest.getActivity().getName(),
      )
