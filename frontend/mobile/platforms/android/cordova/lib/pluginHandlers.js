...
var handlers = {
  ...
  framework: {
    install: function(obj, plugin, project, options) {
      ...
      var parentDir = obj.parent
        ? path.resolve(project.projectDir, obj.parent)
        : project.projectDir
      var subDir

      if (obj.custom) {
        var subRelativeDir = project.getCustomSubprojectRelativeDir(
          plugin.id,
          src,
        )
        copyNewFile(
  ...
  "js-module": {
    install: function(obj, plugin, project, options) {
      ...
      if (options && options.usePlatformWww) {
        // CB-11022 copy file to both directories if usePlatformWww is specified
        var platformWwwDest = path.resolve(
          project.platformWww,
          "plugins",
          plugin.id,
          obj.src,
