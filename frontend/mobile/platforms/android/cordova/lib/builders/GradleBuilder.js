...
GradleBuilder.prototype.runGradleWrapper = function(gradle_cmd, gradle_file) {
  ...
  if (fs.existsSync(gradlePath)) {
    ...
    return superspawn
      .spawn(gradle_cmd, ["-p", this.root, "wrapper", "-b", gradle_file], {
        stdio: "pipe",
      })
...
GradleBuilder.prototype.prepBuildFiles = function() {
  ...
  var pluginBuildGradle = path.join(
    this.root,
    "cordova",
    "lib",
    "plugin-build.gradle",
  ...
  var buildGradle = fs.readFileSync(
    path.join(this.root, "build.gradle"),
    "utf8",
  )
  var depsList = ""
  var root = this.root
...
GradleBuilder.prototype.build = function(opts) {
  ...
  return superspawn
    .spawn(wrapper, args, { stdio: "pipe" })
    .progress(function(stdio) {
      suppressJavaOptionsInfo(stdio)
    })
    .catch(function(error) {
      if (
        error.toString().indexOf("failed to find target with hash string") >= 0
      ) {
  ...
  return Q()
    ...
    .then(function() {
      ...
      ;["debug", "release"].forEach(function(config) {
        var propertiesFilePath = path.join(
          builder.root,
