  ...
  return spawn("adb", ["devices"], { cwd: os.tmpdir() }).then(function(output) {
    return output
      ...
      .filter(function(line) {
        ...
        return line && opts && opts.emulators
          ? isEmulator(line)
          : isDevice(line)
      })
      .map(function(line) {
...
Adb.install = function(target, packagePath, opts) {
  events.emit(
    "verbose",
    "Installing apk " + packagePath + " on target " + target + "...",
  )
  var args = ["-s", target, "install"]
  if (opts && opts.replace) args.push("-r")
  return spawn("adb", args.concat(packagePath), { cwd: os.tmpdir() }).then(
    function(output) {
      ...
      if (output.match(/Failure/)) {
        ...
        return Q.reject(
          new CordovaError("Failed to install apk to device: " + output),
        )
      }
    },
  )
...
Adb.uninstall = function(target, packageId) {
  events.emit(
    "verbose",
    "Uninstalling package " + packageId + " from target " + target + "...",
  )
...
Adb.shell = function(target, shellCommand) {
  ...
  return spawn("adb", args.concat(shellCommand), { cwd: os.tmpdir() }).catch(
    function(output) {
      return Q.reject(
        new CordovaError(
...
Adb.start = function(target, activityName) {
  events.emit(
    "verbose",
    'Starting application "' + activityName + '" on target ' + target + "...",
  )
  return Adb.shell(
    target,
    "am start -W -a android.intent.action.MAIN -n" + activityName,
